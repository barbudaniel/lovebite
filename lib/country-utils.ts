...
// Country coordinates database (latitude, longitude for country centroids)
const countryCoordinates: Record<string, { lat: number; lng: number }> = {
  ...
  MK: { lat: 41.512386, lng: 21.745275 },
...
// Common country name aliases to ISO code mapping
...
export function getCountryCode(countryName: string): string | null {
  ...
  // Check aliases first
  if (countryNameAliases[normalizedName]) {
    return countryNameAliases[normalizedName];
  }
  
  // Try to get code from library
  const code = countries.getAlpha2Code(countryName, "en");
  if (code) return code;
  
  // Try with capitalized name
  const capitalizedName = countryName
    .split(" ")
    .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(" ");
  
  const codeFromCapitalized = countries.getAlpha2Code(capitalizedName, "en");
  if (codeFromCapitalized) return codeFromCapitalized;
...
/**
 * Get country coordinates (latitude, longitude) from country name
 */
...
/**
 * Get full country info from country name
 */
 ...
 */
export function hasValidCoordinates(countryName: string): boolean {
